...
local differenceInner = function() end
...
_.countBy = function(collection, iteratee, selfArg)
	...
	for k, v in _.iter(collection) do
		local r = _.str(callIteratee(iteratee, selfArg, v, k, collection))
		if _.isNil(t[r]) then
			t[r] = 1
		else
			t[r] = t[r] + 1
	...
	for i, v in ipairs(_.table(...)) do
		...
		elseif _.isTable(v) then
			for k2, v2 in _.iter(v) do
				if _.isFunction(v2) then
					_.push(transforms, v2)
				end
			end
		end
	end
...
_.cast = function(a, b)
	...
	local cast
	if _.isString(a) then
		cast = _.str
	elseif _.isBoolean(a) then
		cast = _.bool
	elseif _.isNumber(a) then
		cast = _.num
	elseif _.isFunction(a) then
		cast = _.func
	elseif _.isTable(a) then
		cast = _.table
	end
	return a, cast(b)
end
